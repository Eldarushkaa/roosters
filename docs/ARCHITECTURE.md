...
Текущая версия v4: 2 секунды подготовки/рулетки, 10 секунд боя, до 90
касаний без ограничения скорости и без выбора стоек. При создании фиксируются
`stake_minor`, `win_payout_minor`, случайное число, время и оба снимка.
Серверная запись предшествует анимации. Обновление страницы возвращает тот же
бой, соперника и приз; клиент не может повторно прокрутить рулетку.

Бот имеет целую мощь от 70% до 140% мощности игрока. Его формула вероятности
отдельна от PvP: она даёт теоретический RTP от 90% при нуле касаний до 105%
при девяноста, с малым уменьшением из-за округления фиксированного приза.
PvP использует обычные относительные эффективные мощности, а приз равен
95% общего банка двух одинаковых ставок. Все формулы и определения валовой
...
касаний. В v4 одну пачку из 90 можно принять сразу после старта; защиты от
...
15 монет один раз, вне RTP и счётчика платных боёв для приглашения.

Движки `rules_v1.py`, `rules_v2.py` и `rules_v3.py` сохраняют прежние длительность, вероятность,
...
В текущей v4 сервис отдельно вызывает `bot_probability` для ботов и
`battle_probability` для PvP. Общий метод `rewards` v4 возвращает только XP:
...
Для v3/v4 сервис также возвращает `current_win_probability` по сохранённым
...
Постоянная карточка и история по-прежнему берутся из серверного состояния.
...
Подбор и создание матча проходят под одной блокировкой записи SQLite. Ставки списываются только при найденной паре; подбор требует одинаковую ставку и мощь в пределах коэффициента 1,35. Тестовые игроки встречаются только с тестовыми. Нельзя заменить отсутствующего живого игрока ботом без явного выбора режима: клиент показывает тип соперника.
...
- Проверять отрицательный баланс, точное округление, RTP и возврат общего банка, повтор UUID, изменённый payload, потерю ответа, расход права первого боя и конкурирующие вкладки.
