...
Текущая версия v6: 2 секунды подготовки/рулетки, 10 секунд боя, до 90
касаний без ограничения скорости и без выбора стоек. При создании фиксируются
`stake_minor`, `win_payout_minor` стороны A, `stake_b_minor`,
`win_payout_b_minor` стороны B, случайное число, время и оба снимка.
...
отдельна от PvP: она даёт теоретический RTP от 90% при нуле касаний до 110%
...
приз равен `floor(1.9 × собственная ставка)` независимо от ставки соперника.
Мощь и клики влияют на вероятность и матожидание выплаты. Все формулы и определения валовой
...
касаний. В v4..v6 одну пачку из 90 можно принять сразу после старта; защиты от
...
Движки `rules_v1.py`, `rules_v2.py`, `rules_v3.py`, `rules_v4.py` и `rules_v5.py` сохраняют прежние длительность, вероятность,
...
В текущей v6 сервис отдельно вызывает `bot_probability` для ботов и
`battle_probability` для PvP. Общий метод `rewards` v6 возвращает только XP:
...
Для v3..v6 сервис также возвращает `current_win_probability` по сохранённым
...
предел `floor((2^53−1) × 25/54)` гарантирует точную передачу максимального
приза бота как JSON integer. Для балансов около предела SQLite максимум
дополнительно учитывает запас под выигрыш до 116% ставки. Обычный игровой
