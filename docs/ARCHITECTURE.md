...
Боевой ввод выделен в `web/battle-input.mjs`: мышь, перо и каждый палец дают
удар на `pointerdown`, без ожидания отпускания или распознавания `click`.
Только площадка удара использует `touch-action: none`; детали боя остаются
прокручиваемыми. Последующий указательный `click` не дублирует удар, а
клавиатурная/ассистивная активация и fallback без Pointer Events сохранены.
Анимация локальная; счётчик и шанс показывают только подтверждённые сервером
касания. Подготовка, дедлайн и предел касаний проверяются прежним путём.

Клиент хранит одну неподтверждённую команду с неизменными UUID и JSON.
Временный сбой боевой команды запускает повтор; следующие касания копятся
в буфере. Если `/state` уже показал активный бой после потери ответа
`/battle/start` или `/queue/join`, ввод доступен, а исходная команда
восстанавливается автоматически. Покупки так автоматически не повторяются.
Ожидание ответа боевого повтора увеличивается с 2,5 до 5 и максимум 8 секунд,
чтобы медленное соединение не отменяло каждый ответ на одном коротком пороге.
Это не обещает приём офлайн-касаний: сервер принимает пакеты только до
окончания боя; при длительной потере сети буфер может не успеть дойти.
