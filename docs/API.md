...
```json
{
  ...
  "economy": {
    ...
    "online_quotes": [
      {"stake_minor": 1000, "win_payout_minor": 1900},
      {"stake_minor": 2500, "win_payout_minor": 4750},
      {"stake_minor": 5000, "win_payout_minor": 9500},
      {"stake_minor": 10000, "win_payout_minor": 19000}
    ]
  },
...
Технический потолок ставки — `floor((2^53 − 1) × 25 / 54)` сотых долей:
даже максимальный приз бота `2.16 × ставка` остаётся точным JSON/JavaScript
...
`floor((2^63 − 1 − balance_minor) × 25 / 29)`. Поэтому окончательный
...
```text
{
  ...
  current_win_probability: null | number, // server odds from accepted taps, v3..v6
...
Для v5/v6 online приз равен `floor(1.9 × собственная ставка)` и не зависит от
...
кликам для v3..v6; у v1/v2 это `null`. Поле вычисляется и во время подготовки,
...
Касания v4..v6 принимаются только при `starts_at <= server_time < ends_at`.
...
`state.rules_version` и `catalog` описывают текущие правила v6. При этом
`battle.rules_version` может оставаться `v3`, `v4` или `v5`: такие бои сохраняют
прежние снимки мощи, приз и формулу шанса. У v3 лимит 80 кликов; у v4 — 90
и прежний верхний RTP бота 105%. Для интерфейса исторического боя нужно
...
условия v5, его движок остаётся в реестре.
...
операции; свежий state всегда использует текущий контракт v6. Старую страницу
