...
def bot_probability(player_power: int, bot_power: int, taps: int) -> Fraction:
    """Exact bot win chance, increasing by one sixth after eighty taps.

    The zero-tap probability is P / (P + B). Each accepted tap adds 1/480 of
    that baseline, capped at eighty taps. With a fixed pre-fight prize this
...
def win_probability(
    ...
    Eighty taps provide at most 20% extra effective power. There is no tap-rate
    limit. A 10%..90% probability clamp keeps either side able to win. The
    integer weights below are the common denominator of 1 + taps / 400.
    """
