"""Frozen v1 rules for settling battles created before the v2 release.
...
STANCES = [
    {"id": "rush", "name": "Натиск", "beats": "feint"},
    {"id": "guard", "name": "Защита", "beats": "rush"},
    {"id": "feint", "name": "Обман", "beats": "guard"},
]

_BREED_POWER = {breed["id"]: breed["power"] for breed in BREEDS}
_SLOT_POWER = {slot["id"]: slot["power_per_level"] for slot in SLOTS}
_COUNTERS = {stance["id"]: stance["beats"] for stance in STANCES}
...
def power(breed_id: str, gear: Mapping[str, int], xp: int) -> int:
    """Return base power; battle taps and stance never alter this saved stat.
...
def win_probability(
    ...
    """Compute A's chance; swapping the complete sides gives its complement.

    Sixty accepted taps provide at most 20% extra effective power. The winning
    stance adds a further 12% to its side's effective power, not 12 percentage
    points to the final probability. The service applies tap-rate limits before
    passing accepted counts here. A 10%..90% clamp keeps every battle uncertain.
    """
    _integer(power_a, "power_a", 1)
    _integer(power_b, "power_b", 1)
    _integer(taps_a, "taps_a")
    _integer(taps_b, "taps_b")
    if stance_a not in _COUNTERS or stance_b not in _COUNTERS:
    ...
    if _COUNTERS[stance_a] == stance_b:
        effective_a *= 1 + STANCE_BONUS
    elif _COUNTERS[stance_b] == stance_a:
        effective_b *= 1 + STANCE_BONUS
...
def battle_probability(
    ...
    """Settle an old snapshot using its original power and stance modifiers."""
    return win_probability(
        snapshot_a["power"], snapshot_b["power"],
        snapshot_a["stance"], snapshot_b["stance"], taps_a, taps_b,
    )
